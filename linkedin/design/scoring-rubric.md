...
| HTTP 402 agent payments (x402 / pay-per-crawl) | `agent-ready` | **1** | Body mentions `x402`, `x402.org`, "payment-required header", "pay per crawl", "pay-per-crawl", `crawler-price`, or "x402 foundation" | `Net @endpoint`-scoped budgets bound where payments can flow; `requires { amount <= budget }` gates the payload; IFC labels keep the signed payment key out of public sinks. Umbrella signal covers Coinbase's x402 + Cloudflare's pay-per-crawl (Cloudflare joined the x402 Foundation; both use HTTP 402 with different headers). |
